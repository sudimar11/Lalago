const USERS = 'users';
const USER_ROLE_CUSTOMER = 'customer';
const USER_ROLE_ADMIN = 'admin';
const USER_ROLE_TEACHER = 'teacher';
const USER_ROLE_DRIVER = 'driver';

// Firestore collections
const DAILY_SUMMARIES = 'daily_summaries';
const CREDIT_ENTRIES = 'credit_entries';
const GROUP_CHAT = 'group_chat';
const GROUP_CHAT_TYPING = 'group_chat_typing';
const ADVERTISEMENTS = 'advertisements';
const STORAGE_ADS = 'advertisements';
const DRIVER_COLLECTIONS = 'driver_collections';
const COUPONS = 'coupons';
const STORAGE_COUPONS = 'coupons';
const REPORTS = 'reports';
const CUSTOMER_SUGGESTIONS = 'customer_suggestions';
const CUSTOMER_FEEDBACK = 'customer_feedback';
const SEARCH_ANALYTICS = 'search_analytics';