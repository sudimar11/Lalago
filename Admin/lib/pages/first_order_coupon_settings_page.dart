...
import 'package:brgy/pages/first_order_coupon_usage_page.dart';
import 'package:intl/intl.dart';

class FirstOrderCouponSettingsPage extends StatefulWidget {
  const FirstOrderCouponSettingsPage({super.key});

  @override
  State<FirstOrderCouponSettingsPage> createState() =>
      _FirstOrderCouponSettingsPageState();
}

class _FirstOrderCouponSettingsPageState
    extends State<FirstOrderCouponSettingsPage> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('First Order Coupon Settings'),
        backgroundColor: Colors.black,
        foregroundColor: Colors.white,
      ),
      body: StreamBuilder<FirstOrderCouponConfig>(
        stream: FirstOrderCouponService.getCouponConfigStream(),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                config.enabled
                                    ? 'Enabled - Coupon will be auto-applied to first orders'
                                    : 'Disabled - Coupon is inactive',
                                style: TextStyle(
                        ...
                        Switch(
                          ...
                          onChanged: (value) async {
                            try {
                              await FirstOrderCouponService.updateMasterToggle(
                                  value);
                              if (mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      value
                                          ? 'First Order Coupon enabled'
                                          : 'First Order Coupon disabled',
                                    ),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'View detailed statistics about coupon usage',
                          style: TextStyle(
                            color: Colors.grey[600],
                            fontSize: 14,
                          ),
                          ...
                          child: ElevatedButton.icon(
                            onPressed: () {
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) =>
                                      const FirstOrderCouponUsagePage(),
                                ),
                              );
                            },
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        _CouponConfigForm(config: config),
                      ],
                    ),
                  ),
                ),
...
class _CouponConfigForm extends StatefulWidget {
  final FirstOrderCouponConfig config;

  const _CouponConfigForm({required this.config});

  @override
  State<_CouponConfigForm> createState() => _CouponConfigFormState();
}

class _CouponConfigFormState extends State<_CouponConfigForm> {
  ...
  Future<void> _saveConfig() async {
    ...
    try {
      ...
      final config = FirstOrderCouponConfig(
      ...
      await FirstOrderCouponService.updateCouponConfig(config);
