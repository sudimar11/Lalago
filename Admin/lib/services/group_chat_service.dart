  ...
  static Future<void> _sendNotificationsToOtherDrivers(
    ...
    try {
      ...
      for (final driverDoc in driversSnapshot.docs) {
        ...
        // Try to send FCM notification if method exists
        // Note: Admin app may not have FCM support yet, so we'll skip silently
        try {
          // This would require adding sendFcmMessage to Admin's FirebaseHelper
          // For now, notifications will be handled by Cloud Functions or driver app
        } catch (e) {
          // Silently fail if FCM is not available
          debugPrint('FCM notification not available: $e');
        }
      }
    } catch (e) {
