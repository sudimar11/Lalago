...
class ConfirmedPayoutsPage extends StatefulWidget {
  const ConfirmedPayoutsPage({super.key});

  @override
  State<ConfirmedPayoutsPage> createState() => _ConfirmedPayoutsPageState();
}

class _ConfirmedPayoutsPageState extends State<ConfirmedPayoutsPage> {
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  // Collect all confirmed payout requests
                  final List<ConfirmedPayout> payouts = [];
                  for (var doc in snapshot.data?.docs ?? []) {
                    final data = doc.data() as Map<String, dynamic>;
                    final payoutRequests =
                        data['payoutRequests'] as List<dynamic>? ?? [];
                    ...
                    for (var requestData in payoutRequests) {
                      if (requestData is Map<String, dynamic>) {
                        ...
                        if (status == 'confirmed' &&
                            type == 'earning_wallet_payout') {
                          // Note: Incentive data is not displayed in UI but functionality is preserved
                          // If incentive exists in requestData, it's included in amount calculation but not shown separately
                          final baseAmount =
                              (requestData['amount'] as num?)?.toDouble() ??
                                  0.0;
                          // Incentive field exists but is hidden from display (functionality preserved)
                          // ignore: unused_local_variable
                          final incentive = requestData['incentive'] as num?;

                          payouts.add(ConfirmedPayout(
                            userId: doc.id,
                  ...
                  List<ConfirmedPayout> filteredPayouts = payouts;
                  if (_selectedDate != null) {
                    filteredPayouts = payouts.where((payout) {
                      if (payout.confirmedAt == null) return false;
                      final confirmedDate = payout.confirmedAt!.toDate();
                        ...
                        children: [
                          ...
                          Text(
                            _selectedDate == null
                                ...
                                : 'No payouts found for selected date',
                            style: const TextStyle(
                              fontSize: 18,
                              color: Colors.grey,
                            ),
                          ...
                          child: Table(
                            ...
                            children: [
                              TableRow(
                                children: [
                                  _DataCell('${index + 1}'),
                                  _DataCell(payout.driverName),
                                  _DataCell(
                                    '₱${payout.amount.toStringAsFixed(2)}',
                                  ),
                                  _DataCell(_formatDate(payout.createdAt)),
                                  _DataCell(
                                    _formatDate(payout.confirmedAt),
                                  ),
                                  _DataCell(
                                    'Confirmed',
                                    color: Colors.blue,
