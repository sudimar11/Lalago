...
class InactiveCustomersPage extends StatefulWidget {
  const InactiveCustomersPage({super.key});

  @override
  State<InactiveCustomersPage> createState() => _InactiveCustomersPageState();
}

class _InactiveCustomersPageState extends State<InactiveCustomersPage> {
  int? _zeroOrderCount;
  bool _zeroOrderCountLoading = false;
  int _zeroOrderProgressCurrent = 0;
  int _zeroOrderProgressTotal = 0;
  final ScrollController _scrollController = ScrollController();

  @override
  void initState() {
    super.initState();
    // Load zero order count immediately after first frame
  ...
  Future<void> _loadZeroOrderCount() async {
    setState(() {
      _zeroOrderCountLoading = true;
      _zeroOrderProgressCurrent = 0;
      _zeroOrderProgressTotal = 0;
    });

    try {
      final service = OrderStatsService();
      final count = await service.countActiveCustomersWithZeroOrders(
        onProgress: (current, total) {
          if (!mounted) return;
          setState(() {
            _zeroOrderProgressCurrent = current;
            _zeroOrderProgressTotal = total;
          });
        },
      );
      if (!mounted) return;
      setState(() {
        _zeroOrderCount = count;
      });
    } catch (e) {
      developer.log(
        "Error loading zero order count: $e",
        name: "InactiveCustomersPage",
        error: e,
      );
      if (!mounted) return;
      setState(() {
        _zeroOrderCount = null;
      });
    } finally {
      if (!mounted) return;
      setState(() {
        _zeroOrderCountLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final DateTime thirtyDaysAgo =
        DateTime.now().subtract(Duration(days: 30)).toUtc();
        ...
        builder: (context, customersSnapshot) {
          ...
          return StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, ordersSnapshot) {
              ...
              // Calculate inactive customers = total customers - active customers
              final inactiveCount =
                  allCustomerIds.length - activeCustomerIds.length;
