...
/// Words are stored with verified=false for future admin review.
...
class TausugTeachingsService {
  ...
  /// Teaching trigger phrases (case-insensitive).
  static const _triggers = ['correction', 'teach', 'learn', 'teaching'];

  /// Returns true if the message looks like a teaching intent.
  ...
  /// Parses teaching message to extract Tausug word and English meaning.
  ...
  static TeachingResult? parseTeachingMessage(String message) {
    ...
    // Pattern: "X means Y" or "X is called Y" (Y can have spaces, e.g. "rice wrap")
  ...
  /// session cache. Session cache takes precedence for immediate use.
  static Future<String> getTeachingsHintForQuery(
    ...
    // Add session cache first (taught this session)
