...
class _TableOrderDetailsScreenState extends State<TableOrderDetailsScreen> {
  ...
  void initState() {
    ...
    if (widget.bookTableModel.status == ORDER_STATUS_PLACED) {
      ...
      // Define the PolylineRequest with start and end points and mode
      PolylineRequest request = PolylineRequest(
        ...
        mode: TravelMode.driving, // Set the mode (e.g., driving, walking)
      );

      // Call getRouteBetweenCoordinates with named parameters
      polyLinesFuture = polylinePoints.getRouteBetweenCoordinates(
        googleApiKey: "AIzaSyBXNXXV60p-VYnIMD0mevMk8HeW9kSJnPs",
        request: request,
      );
                        ...
                        child: Column(
                          children: [
                            ...
                            buildDetails(
                              ...
                              title: "Discount",
                              value:
                                  "${widget.bookTableModel.discount} ${widget.bookTableModel.discountType == "amount" ? currencyModel!.symbol : "%"} off",
                            ),
                          ],
                        ),
                      )
                              ...
                              onTap: () {
                                if (widget.bookTableModel.vendor.phonenumber
                                  ...
                                  final Uri launchUri = Uri(
                                    scheme: 'tel',
                                    path: widget
                                        .bookTableModel.vendor.phonenumber,
                                  );
                                  launchUrl(launchUri);
                                }
                              },
