...
class _OffersScreenState extends State<OffersScreen> {
  ...
  getData() async {
    try {
      ...
      List<OfferModel> publicCoupons =
          await FireStoreUtils().getPublicCoupons();

      // Clear previous data
      offersList.clear();
      offerVendorList.clear();

      // Compare coupons with vendors
      for (var offer in publicCoupons) {
        for (var vendor in widget.vendors) {
          if (offer.restaurantId == vendor.id &&
              offer.expireOfferDate != null &&
              offer.expireOfferDate!.toDate().isAfter(DateTime.now())) {
          ...
          children: [
            ...
            Expanded(
              child: offerVendorList.isEmpty
                  ? showEmptyState('No Promo Found', context)
                  : ListView.builder(
