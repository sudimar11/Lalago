...
//
//                       Visibility(
//                         child: Text("Your new delivery charge will be" + " ${amountShow(amount: deliveryCharges.toString())}",
//                             style: TextStyle(
//                               fontFamily: "Poppinsm",
//                             )),
...
//       // } else
//       // {
//       //   showProgress(context, 'Saving Address...', false);
//       //
...
//       //                 builder: (_) {
//       //                   return AlertDialog(
//       //                     content: Text("Please select current address using GPS location. Move pin to exact location"),
//       //                     actions: [
//       //                       // FlatButton(
...
class _DeliveryAddressScreenState extends State<DeliveryAddressScreen> {
  ...
  Future<void> getListAddress() async {
    if (MyAppState.currentUser != null) {
      ...
      // Remove duplicate addresses (based on full address string)
      List<AddressModel> uniqueAddresses = [];
      final seen = <String>{};

      for (var address in updatedUser?.shippingAddress ?? []) {
        if (!seen.contains(address.getFullAddress())) {
          seen.add(address.getFullAddress());
          uniqueAddresses.add(address);
        }
      }

      setState(() {
        shippingAddress = uniqueAddresses;
      });
    }
  }
                  ...
                  return InkWell(
                    onTap: () async {
                      ...
                      shippingAddress.forEach((element) {
                        AddressModel addrModel = element;
                        if (addrModel.id == addressModel.id) {
                          addrModel.isDefault = true;
                        } else {
                          addrModel.isDefault = false;
                        }
                        tempShippingAddress.add(addrModel);
                      });
                      ...
                      hideProgress();
                      // Return the resolved default address, not just the tapped one
                      Navigator.pop(context, resolvedDefaultAddress ?? addressModel);
                    },
                    child: Padding(
        ...
        actions: <CupertinoActionSheetAction>[
          ...
          CupertinoActionSheetAction(
            onPressed: () async {
              ...
              final addressToDelete = shippingAddress[index];
              ...
              // If the deleted address was default, re-determine the new default address
              if (wasDefault && shippingAddress.isNotEmpty) {
                final resolvedDefaultAddress = MyAppState.resolveDefaultAddress(
                    MyAppState.currentUser!.shippingAddress);
                if (resolvedDefaultAddress != null) {
                  MyAppState.selectedPosition = resolvedDefaultAddress;
