...
class UserPreference {
  ...
  static Future init([SharedPreferences? prefs]) async {
    // If prefs is provided, use it. Otherwise, try to get it.
    if (prefs != null) {
      _preferences = prefs;
      return;
    }
    
    final obtainedPrefs = await SharedPreferencesHelper.getInstanceSafe();
    if (obtainedPrefs == null) {
      throw Exception("SharedPreferences unavailable");
    }
    _preferences = obtainedPrefs;
  }
  ...
  static const _lastCheckInDate = "lastCheckInDate";

  static setLastCheckInDate({required String date}) {
    _preferences.setString(_lastCheckInDate, date);
  }

  static getLastCheckInDate() {
    final String? date = _preferences.getString(_lastCheckInDate);
    return date;
  }
}
