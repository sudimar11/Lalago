/// Pure helper for order ready-at and leave-by time calculations.
/// Used by Rider (and Admin uses same parsing for "Ready at" display).
class OrderReadyTimeHelper {
  ...
  /// leaveBy = readyAt − (distance km × minPerKm). Rider only.
  static DateTime getLeaveBy(
    DateTime readyAt,
    double distanceKm, {
    double minPerKm = 2.5,
  }) {
    final minutes = (distanceKm * minPerKm).round();
    return readyAt.subtract(Duration(minutes: minutes));
  }
}
