...
class _LanguageChooceScreenState extends State<LanguageChooseScreen> {
  ...
  void initState() {
    ...
    // Load SharedPreferences after first frame
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        SharedPreferences? sp = await SharedPreferencesHelper.getInstanceSafe();
        if (sp != null && sp.containsKey("languageCode")) {
          setState(() {
              ...
              try {
                ...
                if (sp == null) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Error: Could not access preferences'),
                ...
                // Save language preference (localization removed, but keeping preference storage)
                await sp.setString("languageCode", selectedLanguage);
