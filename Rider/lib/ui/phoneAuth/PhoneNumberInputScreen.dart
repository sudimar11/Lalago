...
class _PhoneNumberInputScreenState extends State<PhoneNumberInputScreen> {
      ...
      if (_verificationID != null) {
        ...
        if (result != null && result is User) {
          if (result.active) {
            ...
            showAlertDialog(
                context, "notLogIn", 'User not activated yet.', true);
          }
        } else if (result != null && result is String) {
          showAlertDialog(context, 'Failed', result, true);
        } else {
          showAlertDialog(
              context, 'Failed', 'Could not create phone number. Please try again.', true);
        }
      } else {
        await hideProgress();
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text("NotVerificationID"),
          duration: Duration(seconds: 6),
        ));
      }
    } on auth.FirebaseAuthException catch (exception) {
      hideProgress();
      String message = "errorOccurredTryAgain";
      switch (exception.code) {
        case 'invalid-verification-code':
          message = "InvalidCodeExpired";
          break;
        case 'user-disabled':
  ...
  _onCameraClick(bool isUserImage) {
    ...
    final action = CupertinoActionSheet(
      message: Text(
        isUserImage ? 'Add profile picture' : 'Add Car Image',
  ...
  _signUp() async {
    if (_key.currentState?.validate() ?? false) {
      ...
      if (_isPhoneValid)
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text("InvalidPhoneTryAgain"),
        ));
    } else {
      setState(() {
