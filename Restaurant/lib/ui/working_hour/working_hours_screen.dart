...
class _WorkingHoursScreenState extends State<WorkingHoursScreen> {
                                ...
                                children: [
                                  ...
                                  InkWell(
                                    ...
                                    child: Icon(Icons.add_circle_sharp,
                                        color: Color(COLOR_PRIMARY), size: 36),
                                  )
                                ],
                              ),
                              ListView.builder(
            ...
            if (MyAppState.currentUser!.vendorID.isEmpty) {
              ...
              workingHours.forEach((element) {
                ...
                var emptyList = element.timeslot!.where(
                    (element) => element.from!.isEmpty || element.to!.isEmpty);
  ...
  saveWorkingHours() async {
    // Remove focus to avoid input conflicts
    FocusScope.of(context).unfocus();

    if (vendorModel != null) {
      vendorModel!.workingHours = workingHours;

      // Debugging: Log the working hours before updating
      print("Saving working hours for vendor: ${vendorModel!.id}");
      print("Working Hours: ${vendorModel!.workingHours}");
      ...
      await FireStoreUtils.updateVendor(vendorModel!).then((value) async {
        ...
        // Navigate to ContainerScreen with Orders as the default screen
        ...
        // Debugging: Catch and log any errors
        print("Error updating working hours: $error");

        hideProgress();
        final snackBar = SnackBar(
          content: Text('Failed to update working hours. Please try again.'),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
      });
    } else {
      // Debugging: Log if vendorModel is null
      print("VendorModel is null. Cannot update working hours.");
      final snackBar = SnackBar(
        content: Text('Failed to update working hours. Vendor not found.'),
      );
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
    }
  }
